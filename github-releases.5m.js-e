 ...
 * [{url: "https://api.github.com/repos/big-neon/bn-api/tags", name: "big-neon/bn-api" }]
 ...
 */
function parseTags(resultData) {
	const {jsonString, name} = resultData;
	let json = JSON.parse(jsonString);
	let repoName = name;
	let pull = json.shift();
	let row = `${repoName}: ${pull.name} | href=https://github.com/big-neon/bn-api/commit/${pull.commit.url.split('/').pop()}\n`;
	return {result: row};
